...
@if($tab==='pengurus')
<thead><tr><th>Nama Lengkap</th><th>NIP</th><th>Jabatan</th><th>Email</th><th>Phone</th><th>Role</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
@foreach($pengurus as $u)<tr><td><strong>{{ $u->name }}</strong></td><td>{{ $u->nip }}</td><td>{{ $u->jabatan }}</td><td>{{ $u->email }}</td><td>{{ $u->phone }}</td><td>{{ $u->role }}</td><td><span class="crm-badge">{{ $u->status_aktif?'Aktif':'Nonaktif' }}</span></td><td><a href="{{ route('crm.edit',[$tab,$u->id]) }}" class="btn btn-sm" style="text-decoration:none">✏️</a> <form method="POST" action="{{ route('crm.destroy',[$tab,$u->id]) }}" style="display:inline" onsubmit="return confirm('Nonaktifkan akun ini?')">@csrf@method('DELETE')<button class="btn btn-sm" style="text-decoration:none;background:none;border:none;color:#dc2626">🗑️</button></form></td></tr>@endforeach
</tbody>
@elseif($tab==='mitra')
...
@if($tab==='pengurus') @foreach($pengurus as $u)<article class="mobile-data-card"><div class="mobile-data-card__title">{{ $u->name }}</div><div class="mobile-data-card__meta">{{ $u->jabatan }} · {{ $u->nip }}<br>{{ $u->email }} · {{ $u->phone }} · {{ $u->status_aktif?'Aktif':'Nonaktif' }}</div></article>@endforeach
