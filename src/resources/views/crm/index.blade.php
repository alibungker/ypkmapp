...
@section('content')
<style>
.crm-tabs{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px}.crm-tab{padding:9px 14px;border:1px solid #d1d5db;border-radius:9px;text-decoration:none;color:#374151;font-size:13px;background:#fff}.crm-tab.active{background:#00034a;color:#fff;border-color:#00034a}.crm-badge{display:inline-flex;padding:3px 8px;border-radius:999px;background:#eef7f0;color:#017723;font-size:11px;font-weight:700}.crm-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}@media(max-width:900px){.crm-grid{grid-template-columns:repeat(2,1fr)}}
</style>
...
@if($tab==='pengurus')
<thead><tr><th>Nama Lengkap</th><th>NIP</th><th>Jabatan</th><th>Email</th><th>Phone</th><th>Role</th><th>Status</th></tr></thead><tbody>
@foreach($pengurus as $u)<tr><td><strong>{{ $u->name }}</strong></td><td>{{ $u->nip }}</td><td>{{ $u->jabatan }}</td><td>{{ $u->email }}</td><td>{{ $u->phone }}</td><td>{{ $u->role }}</td><td><span class="crm-badge">Aktif</span></td></tr>@endforeach
</tbody>
@elseif($tab==='mitra')
<thead><tr><th>Instansi</th><th>Kategori</th><th>PIC</th><th>Kontak</th><th>No. MOU</th><th>Dukungan</th><th>Kontribusi</th></tr></thead><tbody>
@foreach($mitra as $m)<tr><td><strong>{{ $m->nama_instansi }}</strong></td><td>{{ ucwords(str_replace('_',' ',$m->kategori)) }}</td><td>{{ $m->pic_nama }}</td><td>{{ $m->pic_email }}<br>{{ $m->pic_phone }}</td><td>{{ $m->no_mou ?: '-' }}</td><td>{{ ucfirst($m->jenis_dukungan) }}</td><td>Rp {{ number_format($m->total_kontribusi,0,',','.') }}</td></tr>@endforeach
</tbody>
@elseif($tab==='relawan')
<thead><tr><th>Nama</th><th>NIK</th><th>Lahir</th><th>JK</th><th>Kontak</th><th>Keahlian</th><th>Ketersediaan</th><th>Jam</th><th>Domisili</th></tr></thead><tbody>
@foreach($relawan as $r)<tr><td><strong>{{ $r->nama_lengkap }}</strong></td><td>{{ substr($r->nik,0,6) }}******{{ substr($r->nik,-4) }}</td><td>{{ $r->tempat_tanggal_lahir }}</td><td>{{ $r->jenis_kelamin }}</td><td>{{ $r->phone }}<br>{{ $r->email }}</td><td>{{ $r->keahlian_utama }}</td><td>{{ ucwords(str_replace('_',' ',$r->status_ketersediaan)) }}</td><td>{{ number_format($r->jam_kontribusi) }}</td><td>{{ $r->domisili_kota }}</td></tr>@endforeach
</tbody>
@else
...
@if($tab==='pengurus') @foreach($pengurus as $u)<article class="mobile-data-card"><div class="mobile-data-card__title">{{ $u->name }}</div><div class="mobile-data-card__meta">{{ $u->jabatan }} · {{ $u->nip }}<br>{{ $u->email }} · {{ $u->phone }}</div></article>@endforeach
